...
class DownloadsAdapter(
    ...
    class ViewHolder(private val binding: ItemDownloadBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(
            ...
            val speedPart = if (item.state == DownloadState.RUNNING && item.speedLabel.isNotEmpty())
                " • ${item.speedLabel}" else ""
            binding.statusText.text = "${item.progressPercent}% • ${item.state}$speedPart"
