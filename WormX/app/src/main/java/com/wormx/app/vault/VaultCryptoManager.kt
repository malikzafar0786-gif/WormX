...
class VaultCryptoManager(context: Context) {
    ...
    /**
     * Resets the vault PIN via the "Forgot PIN?" link. Note: this does not
     * touch [KEY_FILE_SECRET], so previously-hidden files stay encrypted
     * with the same key and remain recoverable once a new PIN is set.
     */
    fun clearPin() {
