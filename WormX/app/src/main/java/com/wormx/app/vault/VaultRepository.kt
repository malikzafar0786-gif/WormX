...
class VaultRepository private constructor(private val context: Context) {
    ...
    fun hideFromGallery(uri: Uri): ImportResult {
        ...
        try {
            ...
            // Photo Picker URIs frequently can't be deleted directly at all
            // (they're a separate virtual/temporary provider, not the real
            // MediaStore item, and throw IllegalArgumentException rather
            // than SecurityException) — catch broadly here so a failed
            // delete never gets mistaken for the import itself failing; the
            // file is already safely copied and encrypted into the vault by
            // this point regardless.
            try {
                resolver.delete(uri, null, null)
            } catch (e: Exception) {
                android.util.Log.w("VaultRepository", "Couldn't delete original for $uri (import still succeeded)", e)
                // The file may still show in the gallery until the user
                // deletes it there too — the vault copy is safe either way.
            }
