        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                client.newCall(request).execute().use { response ->
                    ...
                    listOf(QualityOption(label = "Original", url = url, approxSizeBytes = size, isAudioOnly = isAudio, referer = url))
                }
            } catch (e: Exception) {
                null
            }
...
/**
 * Tries each registered resolver in order; returns null if none recognized
 * the link. Order: cheap direct-file check first, then the FastAPI/yt-dlp
 * backend (the real per-site extraction engine, if configured and running),
 * then the generic og:video meta-tag fallback for everything else.
 */
class QualityResolverRegistry(context: Context) {
    private val resolvers: List<QualityResolver> = listOf(
        DirectLinkResolver(),
        FastApiExtractorResolver(context.applicationContext),
        OpenGraphResolver()
    )
