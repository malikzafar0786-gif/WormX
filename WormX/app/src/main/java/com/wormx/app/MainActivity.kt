...
class MainActivity : AppCompatActivity() {
    ...
    private val resolverRegistry by lazy { QualityResolverRegistry(this) }
    private val headlessSniffer by lazy { HeadlessVideoSniffer(this) }
    private var orbBusy = false
    private var pendingSlowHint: Runnable? = null
    ...
    private fun showServerConfigDialog() {
        ...
        android.app.AlertDialog.Builder(this)
            .setTitle("Extractor server URL")
            .setMessage("On a real phone, use your computer's LAN IP (both on the same Wi-Fi) — not 10.0.2.2, that only works inside an emulator.")
            .setView(input)
            .setPositiveButton("Save") { _, _ ->
                ServerConfig.setBaseUrl(this, input.text.toString())
                android.widget.Toast.makeText(this, "Server URL saved", android.widget.Toast.LENGTH_SHORT).show()
            }
    ...
    private fun startDownload(url: String) {
        ...
        // If this is still going after a few seconds, it's very likely the
        // free-tier extractor server waking up from sleep (can take up to
        // ~40s) rather than the app being stuck — say so. Cancelled the
        // moment the fetch actually finishes (success or failure) so it
        // never fires a stale message after the flow has already moved on.
        pendingSlowHint = Runnable {
            if (orbBusy) binding.heroLabel.text = "Waking up the server… this can take up to 40s"
        }
        android.os.Handler(mainLooper).postDelayed(pendingSlowHint!!, 6000)
    ...
    private fun finishFetch(options: List<QualityOption>) {
        pendingSlowHint?.let { android.os.Handler(mainLooper).removeCallbacks(it) }
        ...
        android.os.Handler(mainLooper).postDelayed({
            ...
            QualityPickerDialog.showIfNeeded(this@MainActivity, options) { chosen ->
                launchDownload(chosen.url, chosen.referer, chosen.title, chosen.fileExtension)
    ...
    private fun launchDownload(url: String, referer: String? = null, title: String? = null, extension: String? = null) {
        val guessedName = when {
            title != null -> {
                val safeTitle = title.replace(Regex("[\\\\/:*?\"<>|]"), "_").take(80)
                val ext = extension?.trim('.') ?: "mp4"
                "$safeTitle.$ext"
            }
            else -> url.substringAfterLast('/').substringBefore('?').ifBlank { "file_${System.currentTimeMillis()}" }
        }
