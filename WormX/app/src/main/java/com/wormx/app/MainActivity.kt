...
class MainActivity : AppCompatActivity() {
    ...
    private fun startDownload(url: String) {
        ...
        val fetchAnimator = android.animation.ValueAnimator.ofInt(0, 85).apply {
            duration = 1400
            addUpdateListener {
        ...
        uiScope.launch {
            try {
                ...
                if (options != null) {
                    finishFetch(fetchAnimator, options)
                    return@launch
                }
                ...
                headlessSniffer.sniff(url, binding.hiddenWebViewHost) { foundUrl ->
                    if (foundUrl != null) {
                        finishFetch(fetchAnimator, listOf(QualityOption(label = "Original", url = foundUrl)))
                    } else {
    ...
    private fun finishFetch(fetchAnimator: android.animation.ValueAnimator, options: List<QualityOption>) {
        fetchAnimator.cancel()
        ...
        android.os.Handler(mainLooper).postDelayed({
            ...
            QualityPickerDialog.showIfNeeded(this@MainActivity, options) { chosen ->
                ...
                showPage(home = false) // jump to Downloads so progress is immediately visible
            }
        }, 250)
    }
