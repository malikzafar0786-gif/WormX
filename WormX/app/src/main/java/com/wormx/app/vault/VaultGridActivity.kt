...
class VaultGridActivity : AppCompatActivity() {
    ...
    private val galleryPicker = registerForActivityResult(
        ...
        uris.forEach { uri ->
            val result = repository.hideFromGallery(uri)
            if (result.success) imported++ else lastError = result.errorReason
        }
        val message = when {
            imported == uris.size -> "$imported item${if (imported == 1) "" else "s"} moved to Vault"
            imported > 0 -> "$imported of ${uris.size} imported — rest failed${lastError?.let { " ($it)" } ?: ""}"
            else -> "Couldn't import those files${lastError?.let { " ($it)" } ?: ""}"
        }
